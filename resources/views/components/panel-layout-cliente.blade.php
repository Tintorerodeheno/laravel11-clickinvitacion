...
<body class="bg-gray-100">
    <div x-data="{ sidebarOpen: false }" class="flex h-screen bg-gray-100">
        <!-- Sidebar -->
        <nav class="fixed inset-y-0 left-0 w-64 bg-slate-300 text-stone-900 shadow-2xl transform transition-transform duration-300 sm:relative sm:translate-x-0"
             :class="{ '-translate-x-full': !sidebarOpen, 'translate-x-0': sidebarOpen }">
            <div class="flex items-center justify-center h-16 px-4 bg-slate-300">
                <a href="{{ route('cliente.dashboard') }}" class="flex items-center space-x-2">
                    <x-application-logo class="h-10 w-auto" />
                    
                </a>
                <button @click="sidebarOpen = false" class="absolute right-4 sm:hidden p-2 text-white hover:bg-purple-800 rounded-md">
            ...
            <ul class="mt-4">
                {{-- @foreach($menuItems as $item)
                    @if(is_null($item['permission']) || auth()->user()->can($item['permission']))
                        <li class="px-4 py-2">
                            <a href="{{ $item['route'] }}" class="flex items-center space-x-3 text-white hover:bg-purple-500 rounded-md px-3 py-2">
                                <span class="w-6 h-6 flex justify-center items-center">{!! $item['icon'] !!}</span>
